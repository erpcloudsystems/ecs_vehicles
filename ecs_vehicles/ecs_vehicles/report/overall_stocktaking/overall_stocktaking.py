...
def get_data(filters, columns):
    ...
    list_gas_stocktaking = get_gas_stocktaking(filters)
    list_oil_stocktaking = get_oil_stocktaking(filters)
    list_washing_stocktaking = get_washing_stocktaking(filters)

    return (
        item_price_qty_data
        + list_gas_stocktaking
        + list_oil_stocktaking
        + list_washing_stocktaking
    )


def get_washing_stocktaking(filters):
    ...
    if voucher_type_list:
        for x in voucher_type_list:
            data = {"voucher_type": x.voucher_type}
                    ...
                    frappe.db.count(
                        ...
                        filters=[
                            [
                                "serial_no",
                                "between",
                                [int(item_dict.min), int(item_dict.max)],
                            ],
                            ["voucher_type", "=", x.voucher_type],
                        ],
                    )
                    if int(item_dict.max) - int(item_dict.min) != 0
                    else 0
                )
                data["count_notebook"] = (
                    frappe.db.count(
...
def get_oil_stocktaking(filters):
    voucher_type_list = frappe.db.sql(
        """
        select distinct `tabVoucher`.voucher_type as voucher_type
        ...
        for x in voucher_type_list:
            ...
            for item_dict in item_results:
                ...
                data["total_price"] = (
                    ...
                    * float(item_dict.litre_count)
                    * item_dict.litre_rate
                    if int(item_dict.max) - int(item_dict.min) != 0
                    else 0
                )
...
def get_gas_stocktaking(filters):
    voucher_type_list = frappe.db.sql(
        """
        select distinct `tabVoucher`.voucher_type as voucher_type
        from `tabVoucher` where `tabVoucher`.liquid_type = "غاز" 
        group by `tabVoucher`.voucher_type
        """,
        as_dict=1,
    )
        ...
        for x in voucher_type_list:
            ...
            item_results = frappe.db.sql(
                ...
                select
                    ifnull(max(`tabVoucher`.serial_no),0) as max,
                    ifnull(min(`tabVoucher`.serial_no),0) as min,
                    (select `tabGas Voucher`.gas_count from `tabGas Voucher`
                    where `tabGas Voucher`.name = '{voucher_type}') as qty_litre,
                    (select `tabGas Voucher`.meter_rate from `tabGas Voucher` 
                    where `tabGas Voucher`.name = '{voucher_type}') as unit_price
                ...
                where
                    ...
                    and `tabVoucher`.disabled = 0
                    and `tabVoucher`.serial_no > 0
                    and `tabVoucher`.liquid_type = "غاز"
                    and `tabVoucher`.issue_no is null
            ...
            for item_dict in item_results:
                ...
                data["unit_price"] = item_dict.unit_price
                data["total_price"] = (
                    frappe.db.count(
                        "Voucher",
                        filters=[
...
def get_item_price_qty_data(filters):
    voucher_type_list = frappe.db.sql(
        """
        select distinct `tabVoucher`.voucher_type as voucher_type
        from `tabVoucher` where `tabVoucher`.liquid_type = "وقود" 
        group by `tabVoucher`.voucher_type
        """,
        as_dict=1,
    )
        ...
        for x in voucher_type_list:
            ...
            item_results = frappe.db.sql(
                ...
                select
                    ifnull(max(`tabVoucher`.serial_no),0) as max,
                    ifnull(min(`tabVoucher`.serial_no),0) as min,
                    (select `tabFuel Voucher`.litre_count from `tabFuel Voucher` 
                    where `tabFuel Voucher`.name = '{voucher_type}') as litre_count,
                    (select `tabFuel Voucher`.litre_rate from `tabFuel Voucher` 
                    where `tabFuel Voucher`.name = '{voucher_type}') as litre_rate
                ...
                where
                    `tabVoucher`.voucher_type = '{voucher_type}'
                    and `tabVoucher`.disabled = 0
                    and `tabVoucher`.liquid_type = "وقود"
                    and `tabVoucher`.serial_no > 0
                    and (`tabVoucher`.issue_no is null
                    or `tabVoucher`.issue_no = "")
