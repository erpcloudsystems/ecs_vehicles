...
def get_item_price_qty_data(filters):
    ...
    if filters.get("recruit"):
        conditions += "and  `tabHospital Transaction`.recruit = %(recruit)s"
    if filters.get("start_date"):
        conditions += " and  `tabHospital Transaction`.end_date >= %(start_date)s"
    if filters.get("end_date"):
        conditions += " and  `tabHospital Transaction`.end_date <= %(end_date)s"
    # if filters.get("overnight")== "مبيت":
    #     conditions += " and  `tabHospital Transaction`.types_of_vacations = %(overnight)s"
    ...
    item_results = frappe.db.sql("""
            ...
            from
                `tabHospital Transaction`
            where
                `tabHospital Transaction`.docstatus = 1
                {conditions}
