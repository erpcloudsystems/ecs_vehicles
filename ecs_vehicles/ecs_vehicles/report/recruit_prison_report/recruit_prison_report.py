...
def get_item_price_qty_data(filters):
    ...
    if filters.get("recruit"):
        conditions += "and  `tabPrison Transaction`.recruit = %(recruit)s"
    if filters.get("start_date"):
        conditions += " and  `tabPrison Transaction`.end_date >= %(start_date)s"
    if filters.get("end_date"):
        conditions += " and  `tabPrison Transaction`.end_date <= %(end_date)s"
    # if filters.get("overnight")== "مبيت":
    #     conditions += " and  `tabPrison Transaction`.types_of_vacations = %(overnight)s"
    ...
    item_results = frappe.db.sql("""
            ...
            from
                `tabPrison Transaction`
            where
                `tabPrison Transaction`.docstatus = 1
                {conditions}
