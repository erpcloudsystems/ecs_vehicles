        ...
        for x in voucher_type_list:
            ...
            for item_dict in item_results:
                ...
                data['count_voucher'] = frappe.db.count("Voucher", filters=[
                    ['serial_no', 'between', [int(item_dict.min), int(item_dict.max)]], ['voucher_type', '=', x.voucher_type]]) if int(item_dict.max) - int(item_dict.min) != 0 else 0
                data['count_notebook'] = frappe.db.count("Voucher", filters=[
                    ['serial_no', 'between', [int(item_dict.min), int(item_dict.max)]], ['voucher_type', '=', x.voucher_type]]) / 25 if int(item_dict.max) - int(item_dict.min) != 0 else 0
                data['qty_litre'] = frappe.db.count("Voucher", filters=[
                    ['serial_no', 'between', [int(item_dict.min), int(item_dict.max)]], ['voucher_type', '=', x.voucher_type]]) * float(item_dict.litre_count) if int(item_dict.max) - int(item_dict.min) != 0 else 0
