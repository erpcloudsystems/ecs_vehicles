...
def get_item_price_qty_data(filters):
    ...
    if filters.get("from_date"):
        conditions += " and write_off_date >= %(from_date)s"
    if filters.get("to_date"):
        conditions += " and write_off_date <= %(to_date)s"
    if filters.get("return_date"):
        conditions += "and return_date = %(return_date)s"
    ...
    item_results = frappe.db.sql("""
        select
            recruit, employee_name, military_date, write_off_date, return_date, notes
        from
            `tabRecruit Write Off`
        where
            `tabRecruit Write Off`.docstatus = 1
            {conditions}
    ...
    for item_dict in item_results:
        data = {
            ...
            'write_off_date': item_dict.write_off_date,
            'return_date': item_dict.return_date,
