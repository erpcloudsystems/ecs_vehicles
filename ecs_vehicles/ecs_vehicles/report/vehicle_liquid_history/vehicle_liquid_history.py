...
def get_conditions (filters):
    ...
    if filters.get("from_date"):
        conditions += " AND `tabLiquids Issuing Table`.from_date >= '{0}'".format(filters.get("from_date"))
    if filters.get("to_date"):
        conditions += " AND `tabLiquids Issuing Table`.from_date <= '{0}'".format(filters.get("to_date"))
    return conditions
...
def get_item_price_qty_data(filters):
    ...
    if frappe.db.exists("Vehicles", {"vehicle_no": filters.get("name")}):
        ...
        if filters.get("entity"):
            conditions += " AND `tabVehicles`.entity_name >= '{0}'".format(filters.get("entity"))
        if filters.get("vehicle_type"):
            conditions += " AND `tabVehicles`.vehicle_type >= '{0}'".format(filters.get("vehicle_type"))
        item_results = frappe.db.sql("""
            ...
            order by
                `tabLiquids Issuing Table`.from_date
            """.format(name=filters.get("name"), issue_type=filters.get("issue_type"), conditions=conditions), filters, as_dict=1)
    ...
    # elif frappe.db.exists("Vehicles", {"police_id": filters.get("name")}):
    #     vehicle = frappe.get_doc("Vehicles", {"police_id": filters.get("name")})
    #     if filters.get("entity"):
    #         conditions += " AND `tabVehicles`.entity_name >= '{0}'".format(filters.get("entity"))
    #     if filters.get("vehicle_type"):
    #         conditions += " AND `tabVehicles`.vehicle_type >= '{0}'".format(filters.get("vehicle_type"))
    ...
    #         result.append(data)
        ...
        if filters.get("entity"):
            conditions += " AND `tabBoats`.entity_name >= '{0}'".format(filters.get("entity"))
        item_results = frappe.db.sql("""
            select
                `tabLiquids Issuing Table`.entity,
                `tabLiquids Issuing Table`.vehicle_status,
