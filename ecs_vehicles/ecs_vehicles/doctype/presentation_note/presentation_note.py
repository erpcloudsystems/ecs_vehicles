...
def add_po_f(request_for_quotations,supplier,name, method=None):
        ...
        items = frappe.db.sql(""" select a.item_group,
                            ...
                            """.format(request_for_quotations=request_for_quotations,presentation_note="Presentation Note",parentfield="presentation_note_item"), as_dict=1)
        ...
        for x in items:
            ...
            table.stock_uom = frappe.db.get_value("Item", x.item_code, "stock_uom") # x.default_unit_of_measure
...
class PresentationNote(Document):
    ...
    def create_purchase_import_order(doc, method=None):
        ...
        frappe.msgprint("تم إنشاء أمر توريد رقم <a href=/app/purchase-order/{0}>{0}</a>".format(purchase_order.name))
    @frappe.whitelist()
    def get_data(doc, method=None):
        items = frappe.db.sql(""" select ezn_egraa_item.item_group,
                            ...
                            """.format(maintenance_method="حافظة مشتريات", name=doc.request_for_quotations), as_dict=1)
        for x in items:
    ...
    @frappe.whitelist()
    def validate(self):
        mozakira_no_list = frappe.db.sql(""" Select mozakira_no, name from `tabPresentation Note`
        ...
        for x in mozakira_no_list:
            if self.mozakira_no == x.mozakira_no:
                frappe.throw(
                    " لا يمكن تكرار رقم المذكرة " + str(x.mozakira_no) + " أكثر من مرة قي نفس السنة المالية حيث أنه مستخدم في المستند " + x.name)
