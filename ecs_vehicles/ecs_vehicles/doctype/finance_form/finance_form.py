...
class FinanceForm(Document):
	...
	def get_total_of_invoices(self):
		...
		self.total_price_difference = self.price_difference
	def get_bank_details(self):
		bank = frappe.db.get_list("Banks Table", {"parent":self.supplier, "parenttype":"Supplier", "default":"1"}, ["bank", "account_no"])
		if bank:
			self.bank_name = bank[0].bank
			self.bank_account_no = bank[0].account_no
		elif  frappe.db.exists("Banks Table", {"parent":self.supplier, "parenttype":"Supplier"}):
			bank_name, account_no = frappe.db.get_value("Banks Table", {"parent":self.supplier, "parenttype":"Supplier"}, ["bank", "account_no"])
	...
	def calculate_imprints(self):
		...
		if 50 <= self.total_invoices <= 300 and self.include_imprints :
			self.normal_imprint = (self.total_invoices - 50) * 12 / 100
		...
		if 300 <= self.total_invoices <= 550 and self.include_imprints:
			self.normal_imprint = (self.total_invoices - 50) * 13 / 100
			self.extra_imprint = self.normal_imprint * 3
			self.total_of_imprints = self.normal_imprint + self.extra_imprint
		if 550 <= self.total_invoices <= 1050 and self.include_imprints:
			self.normal_imprint = (self.total_invoices - 50) * 14 / 100
			self.extra_imprint = self.normal_imprint * 3
			self.total_of_imprints = self.normal_imprint + self.extra_imprint
		if 1050 <= self.total_invoices <= 5050 and self.include_imprints:
			self.normal_imprint = (self.total_invoices - 50) * 15 / 100
			self.extra_imprint = self.normal_imprint * 3
			self.total_of_imprints = self.normal_imprint + self.extra_imprint
		if 5050 <= self.total_invoices <= 1050 and self.include_imprints:
			self.normal_imprint = (self.total_invoices - 50) * 16 / 100
			self.extra_imprint = self.normal_imprint * 3
			self.total_of_imprints = self.normal_imprint + self.extra_imprint
	...
	def validate(self):
		if frappe.db.exists("Finance Form", {"series_no": self.series_no, "year": self.year, "name":["!=", self.name]}):
